...
# 2. Auf den unprivilegierten User wechseln
USER 1001
...
# 4. Port freigeben und Datei via EXEC und cat fehlerfrei streamen
EXPOSE 8080
CMD ["/usr/bin/socat", "TCP-LISTEN:8080,reuseaddr,fork", "EXEC:cat /tmp/http_response.txt"]
